<x-layouts::app :title="__('Dashboard')">
    ...
    class="flex">
            ...
            @foreach ($products as $item )
                <section class="bg-slate-50 p-2">
                    ...
                    <div>
                        ...
                        <button 
                        class="bg-emerald-500 px-4 py-2 rounded-md shadow-md"
                        @click="helloWorld({
                            id: {{ $item->id }}, 
                            nama: '{{ $item->nama }}', 
                            harga: {{ $item->harga }} 
        ...
        <section 
            class="min-w-[20svw] bg-slate-200 flex flex-col h-[90svh] 
            sticky top-4">
            <button class="bg-emerald-500 py-2 px-4">Buat Transaksi</button>
            <span class="text-emerald-700" x-text="'Rp ' + cartItems.reduce((acc, i) => (acc + i.harga) * i.quantity, 0).toLocaleString('id-ID')"></span>
            <section class="text-black">   
                <template x-for="(item, index) in cartItems" :key="item.id">
                    <div class="flex justify-between items-center bg-slate-400 p-3 rounded-xl border border-stone-700">
                        <div>
                            <p class="text-black font-medium" x-text="item.nama"></p>
